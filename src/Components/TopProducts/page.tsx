...
// import { Product } from "../../app/context/wishlistContext";
...
//                   {product.tags.map((tag: string | number | bigint | boolean | ReactElement<any, string | JSXElementConstructor<any>> | Iterable<ReactNode> | ReactPortal | Promise<AwaitedReactNode> | Iterable<ReactNode> | null | undefined, index: Key | null | undefined) => (
//                     <span
//                       key={index}
          ...
          >
            ...
            <Link href={`/OurProducts/${product.slug?.current}`}>
              <div>
                ...
                <div className="mt-2 flex flex-wrap gap-2">
                  {product.tags.map((tag: string | number, index: number) => (
                    <span
                      key={index}
                      className="text-xs bg-slate-400 text-black px-2 py-1 rounded-md"
                    >
                      {typeof tag === "string" ? tag : tag.toString()} {/* Convert non-string values to string */}
                    </span>
                  ))}
                </div>
              </div>
